	...
	bool includeSteering)
{
	...
	_includeSimulatorFlags |= ((throttleCount>0) ? JOYSTICK_INCLUDE_THROTTLE : 0);
	...
	uint8_t simulationCount = (includeRudder == true) + _throttleCount + (includeAccelerator == true) + (includeBrake == true) + (includeSteering == true);
	...
	_yAxisRotation = 0;
	_zAxisRotation = 0;
	for (int index = 0; index < JOYSTICK_THROTTLE_COUNT_MAXIMUM; index++)
	{
		_throttle[index] = 0;
	}
	_rudder = 0;
...
void Joystick_::sendState()
{
	...
	// Load Throttle State
	for (int t_index = 0; t_index < _throttleCount; t_index++)
	{
		index += buildAndSetSimulationValue(_includeSimulatorFlags & JOYSTICK_INCLUDE_THROTTLE, _throttle[t_index], _throttleMinimum, _throttleMaximum, &(data[index]));
	}
